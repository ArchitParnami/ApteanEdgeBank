...
{
    public partial class TFSAccountForm : Form
    {
        private CustomerForm parent;
        private DataHandler DH;
        private TFSAccount account;
        ...
        public TFSAccountForm(CustomerForm parent, DataHandler DH, 
                                   TFSAccount account, Customer customer)
        {
            InitializeComponent();
            ...
            typeTextBox.Text = Default.TFSA;

            depositBox.Maximum = Default.TFS_ACCOUNT_BALANCE_LIMIT;
            withdrawBox.Maximum = Default.TFS_ACCOUNT_BALANCE_LIMIT;
            amountBox.Maximum = Default.TFS_ACCOUNT_BALANCE_LIMIT;
        ...
        private void closeAccountButton_Click(object sender, EventArgs e)
        {
            ...
            if (account.IsAccountOpen) // close the account
            {
                ...
                if (account.Balance == 0.0)
                {
                    ...
                    {
                        DH.CloseTFSAccount(customer, account);
                        InitializeUI();
                        statusChanged = true;
                    }
                }
            ...
            else // reopen Account
            {
                ...
                if (result == DialogResult.Yes)
                {
                    DH.ReopenTFSAccount(customer, account);
                    InitializeUI();
                    statusChanged = true;
                }
            }
        ...
        private void TFSAccountForm_FormClosing(object sender, FormClosingEventArgs e)
        {
            if (statusChanged)
        ...
        private void depositOkButton_Click(object sender, EventArgs e)
        {
            if (depositOkButtonClicked)
            {
                ...
                long balanceAfterDeposit = depositAmount + account.Balance;

                if (balanceAfterDeposit > Default.TFS_ACCOUNT_BALANCE_LIMIT)
                {
                    string message = "Transaction Failed: This transaction Exceeds the " +
                        "TFS balance limit of " + Default.TFS_ACCOUNT_BALANCE_LIMIT;
                    MessageBox.Show(message, "Transaction Failed");
                }

                else
                {
                    DH.UpdateTFSAccountBalance(account, depositAmount);
        ...
        private void withdrawOkButton_Click(object sender, EventArgs e)
        {
            if (withdrawOkButtonClicked)
            {
                ...
                {
                    DH.UpdateTFSAccountBalance(account, -amountWithdraw);
        ...
        {
            toAccNo = toAccountTextBox.Text.ToUpper();
            ...
            if (amountOkButtonClicked)
            {
                ...
                if (amount <= account.Balance)
                {
                    ...
                    if (toAccNo.StartsWith("CA")) // chequingAccount
                    {
                        ...
                        DH.UpdateTFSAccountBalance(account, -amount);
        ...
        private void TFSAccountForm_Load(object sender, EventArgs e)
        {
